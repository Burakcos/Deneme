...
namespace AssetManagment.Data
{
	public class FilterFunctions
    {
        ...
        {
            string filter = Regex.Replace(this._filter, "\\$((?\'Name\'\\w+)\\s*\\((?\'Arguments\'[\\s\\S]*?)\\)\\s*)", DoReplaceFunctions);
            return filter;
        }
    ...
    public class FilterFunctionBase
    {
        ...
        {
            Match m = Regex.Match(arguments, "^\\s*(\\w+)\\s*(,|\\$comma\\$)?");
            return m.Groups[1].Value;
        }
        
        protected string ExtractArgument(string arguments, int index)
        {
            Match m = Regex.Match(arguments, "^\\s*(\\w+)\\s*(,|\\$comma\\$)\\s*([\\s\\S]*?)\\s*$");
