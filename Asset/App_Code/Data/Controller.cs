	...
	public partial class DataControllerBase
    {
        ...
        public static ControllerConfiguration CreateConfigurationInstance(Type t, string controller)
        {
            ...
            if (config == null)
            {
                ...
                if (res == DefaultDataControllerStream)
                	res = null;
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("AssetManagment.Controllers.{0}.xml", controller));
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("AssetManagment.{0}.xml", controller));
                if (res == null)
                {
        ...
        public virtual void SelectView(string controller, string view)
        {
            ...
            if (!(ValidateViewAccess(controller, _viewId, accessType)))
            	throw new Exception(String.Format("Not authorized to access private view \'{0}\' in data controller \'{1}\'. Set \'Access" +
                            "\' property of the view to \'Public\'.", _viewId, controller));
        }
        ...
        protected virtual bool ConfigureCommand(DbCommand command, ViewPage page, CommandConfigurationType commandConfiguration, FieldValue[] values)
        {
            ...
            if (command == null)
            	return true;
            if (values != null)
            	AssetManagment.Security.EventTracker.EnsureTrackingFields(page, _config);
        ...
        public static string CreateReportInstance(Type t, string name, string controller, string view, bool validate)
        {
            ...
            if (t == null)
            	t = typeof(AssetManagment.Data.Controller);
            Stream res = t.Assembly.GetManifestResourceStream(String.Format("AssetManagment.Reports.{0}", name));
            if (res == null)
            	res = t.Assembly.GetManifestResourceStream(String.Format("AssetManagment.{0}", name));
            if (res == null)
            {
            ...
            if (isGeneric)
            {
                ...
                ControllerConfiguration config = AssetManagment.Data.Controller.CreateConfigurationInstance(t, controller);
