...
#pragma warning restore 0618
        ...
        ViewPage IDataController.GetPage(string controller, string view, PageRequest request)
        {
            ...
            AssetManagment.Security.EventTracker.Process(page, request);
            if (rules != null)
            ...
            try
            {
                ...
                if (args.SqlCommandType != CommandConfigurationType.None)
                	using (DbConnection connection = CreateConnection())
                    {
                        ...
                        {
                            AssetManagment.Security.EventTracker.Process(args, _config);
        ...
        protected virtual bool ValidateViewAccess(string controller, string view, string access)
        {
            ...
            if (!(executionFilePath.StartsWith("~/appservices/", StringComparison.OrdinalIgnoreCase)) && !(executionFilePath.Equals("~/blob.ashx", StringComparison.OrdinalIgnoreCase)))
            {
                if (!(HttpContext.Current.User.Identity.IsAuthenticated) && !(controller.StartsWith("aspnet_")))
                	allow = (access == "Public");
            }
            return allow;
        }
