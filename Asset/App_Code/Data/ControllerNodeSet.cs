...
namespace AssetManagment.Data
{
	public class ControllerNodeSet
    {
        ...
        private string DoReplaceElementName(Match m)
        {
            ...
            string axis = m.Groups["Axis"].Value;
        ...
        {
            Match m = _createElementRegex.Match(selector);
            if (m.Success)
            {
        ...
        private ControllerNodeSet InternalSelect(bool add, string selector, params System.Object[] args)
        {
            ...
            List<XPathNavigator> rootNodes = _nodes;
            if ((rootNodes.Count == 0) || add)
            {
            	...
            	if (Char.IsLetter(selector, 0) && !(_namespaceRegex.IsMatch(selector)))
                	selector = (".//" + selector);
            foreach (XPathNavigator root in rootNodes)
            	if (args.Length > 0)
                {
                    _args = args;
                    while (_argIndex < args.Length)
                    {
                        string xpath = _variableRegex.Replace(selector, DoReplaceVariable);
                        XPathNodeIterator iterator = root.Select(xpath, _resolver);
                ...
                {
                    XPathNodeIterator iterator = root.Select(selector, _resolver);
                    while (iterator.MoveNext())
                    	list.Add(iterator.Current.Clone());
                }
        ...
        public ControllerNodeSet Attr(string name, object value)
        {
            ...
            if (value is bool)
            	s = s.ToLower();
            foreach (XPathNavigator nav in _nodes)
            {
                XPathNavigator attrNav = nav.SelectSingleNode(("@" + name));
                if (attrNav != null)
                	attrNav.SetValue(s);
                else
        ...
        public ControllerNodeSet Arrange(string selector, params System.String[] sequence)
        {
            ...
            while (i >= 0)
            {
                foreach (XPathNavigator node in _nodes)
                {
                    XPathNavigator seqNav = node.SelectSingleNode(selector, _resolver);
                    if ((seqNav != null) && (seqNav.Value == sequence[i]))
                    {
                        XPathNavigator sibling = node.Clone();
        ...
        {
            string s = Convert.ToString(value);
            string selector = ("c:" + name);
            foreach (XPathNavigator node in _nodes)
            {
                XPathNavigator elemNav = node.SelectSingleNode(selector, _resolver);
                if (elemNav == null)
                {
        ...
        public ControllerNodeSet SelectViews(params System.String[] types)
        {
            if (types.Length == 0)
            	return NodeSet().Select("/dataController/views/view");
            return NodeSet().Select("/dataController/views/view[@type=$type]", types);
        }
        ...
        public ControllerNodeSet SelectActions(params System.String[] commandNames)
        {
            if (commandNames.Length == 0)
            	...
            	return SelectInContext("action", "action[@commandName=$commandName]", commandNames);
        }
        ...
        {
            ControllerNodeSet actionNode = Select("<action/>").AppendTo(this.Select("ancestor-or-self::actionGroup")).Select("action[last()]");
            if (!(String.IsNullOrEmpty(id)))
        ...
        public ControllerNodeSet CreateView(string id, string type, string commandId)
        {
            ...
            {
                XPathNavigator commandIdNav = _navigator.SelectSingleNode("/c:dataController/c:commands/c:command/@id", _resolver);
        ...
        {
            string selector = "/dataController/businessRules/rule";
        ...
        {
            ControllerNodeSet businessRulesNode = Select("/dataController/businessRules");
            if (businessRulesNode.Nodes.Count == 0)
            	businessRulesNode = Select("<businessRules/>").AppendTo("/dataController").Select("businessRules");
            ControllerNodeSet ruleNode = Select("<rule/>").AppendTo(businessRulesNode).Select("rule[last()]");
        ...
        {
            ControllerNodeSet fieldsNode = Select("/dataController/fields");
            ...
            if (String.IsNullOrEmpty(type))
            	type = "String";
            ControllerNodeSet fieldNode = Select("<field/>").AppendTo(fieldsNode).Select("field[last()]").Attr("name", name).Attr("type", type);
            if (type == "String")
        ...
        public ControllerNodeSet StatusBar(string formula, string type, string statusMap)
        {
            ...
            ControllerNodeSet statusBarNode = Select("/dataController/statusBar");
