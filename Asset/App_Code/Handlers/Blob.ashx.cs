...
namespace AssetManagment.Handlers
{
    ...
    public class BlobHandlerInfo
    {
        ...
        public virtual string ContentTypeField
        {
            get
            {
                if (!(String.IsNullOrEmpty(_contentTypeField)))
        ...
        public virtual string LengthField
        {
            get
            {
                if (!(String.IsNullOrEmpty(_lengthField)))
        ...
        private List<FieldValue> CreateActionValues(Stream stream, string contentType, string fileName, int contentLength)
        {
            ...
            if (stream != null)
            {
                XPathNavigator lengthField = config.SelectSingleNode("/c:dataController/c:fields/c:field[@name=\'{0}\']", this.LengthField);
                if (lengthField == null)
                ...
                XPathNavigator contentTypeField = config.SelectSingleNode("/c:dataController/c:fields/c:field[@name=\'{0}\']", this.ContentTypeField);
                if (ContentTypeField == null)
                	contentTypeField = config.SelectSingleNode("/c:dataController/c:fields/c:field[@name=\'{0}ContentType\' or @name=\'{0}CONTENTTYP" +
                ...
                XPathNavigator fileNameField = config.SelectSingleNode("/c:dataController/c:fields/c:field[@name=\'{0}\']", this.FileNameField);
    ...
    public class Blob : GenericHandlerBase, IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        void IHttpHandler.ProcessRequest(HttpContext context)
        {
            ...
            if (handler.Mode == BlobMode.Upload)
                ...
                {
                    if (!(ba.IsPublic) && !(context.User.Identity.IsAuthenticated))
                    {
        ...
        private void CopyToOutput(HttpContext context, Stream stream, BlobHandlerInfo handler)
        {
            ...
            if (handler.Mode == BlobMode.Thumbnail)
            {
                ...
                Rectangle r = new Rectangle(0, 0, (thumbWidth - 1), (thumbHeight - 1));
        ...
        public static void InitializeRow(ViewPage page, object[] row)
        {
            ...
            foreach (DataField field in page.Fields)
            {
                ...
                if (ba != null)
                {
                    ...
                    {
                        if ((utilityField.Name == ba.FileNameField) || ((utilityField.Name == ba.ContentTypeField) || (utilityField.Name == ba.LengthField)))
                        {
                            if (row[utilityFieldIndex] == null)
